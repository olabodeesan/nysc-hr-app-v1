 ...
 * @author IronHide
 */
public class PromotionProposalReportService implements Serializable {

    private String employeeSelectQuery;
    private String compulsoryWhereClause, orderClause;
    private final ServiceUtil serviceUtil;
    private final DBUtil dBUtil;

    public PromotionProposalReportService() {
    ...
    private void initWhereClauses() {        
        ...
        strBuilder.append(" e.present_status IN ")
                .append(" ( ")
                .append(AppConstants.PERSONNEL_ACTIVE).append(",")
                .append(AppConstants.PERSONNEL_SUSPENDED)
                .append(" ) and e.date_due_for_retirement>curdate() ");
        compulsoryWhereClause = strBuilder.toString();
    ...
    public String getCompulsoryWhereClause() {
        return compulsoryWhereClause;
    }
    ...
    private void initSelectQuery() {
        ...
        StringBuilder strBuilder = new StringBuilder();
        strBuilder.append(year).append("-01-01");

        String firstJan = strBuilder.toString();

        //System.out.print(firstJan);
        ...
        strBuilder.append("select e.employee_rec_id,e.file_no, e.surname, e.other_names, e.gender, e.marital_status,sms.description as _marital_status")
                ...
                .append(",timestampdiff(YEAR,e.current_date_of_present_appt,'").append(firstJan).append("') as _year_length_of_stay ")
    ...
    public List<EmployeeDTO> searchRecords(String selectQuery)
        ...
        try {
            ...
            while (result_set.next()) {
                ...
                if(StringUtils.equals(dto.getPresentStatus(), AppConstants.PERSONNEL_SUSPENDED)){
                    dto.setPresentlySuspended("Y");
                }else{
                    dto.setPresentlySuspended("N");
                }

                int _pres_grade_level = result_set.getInt("current_grade_level");
                int _year_length_of_stay = result_set.getInt("_year_length_of_stay");
                
                //System.out.println(_pres_grade_level + "," + _year_length_of_stay);

                if ((_pres_grade_level == 15 || _pres_grade_level == 16) && _year_length_of_stay >= 4) {
                    result.add(dto);
                }else if ((_pres_grade_level >= 7 && _pres_grade_level <= 14) && _year_length_of_stay >= 3) {
                    result.add(dto);
                }else if ((_pres_grade_level >= 3 && _pres_grade_level <= 6) && _year_length_of_stay >= 2) {
                    result.add(dto);
                }
