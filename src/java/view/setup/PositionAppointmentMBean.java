...
import service.setup.PositionAppointmentService;
 ...
 * @author IronHide
 */
public class PositionAppointmentMBean implements Serializable{

    private PositionAppointmentDTO newRecord;
    private List<PositionAppointmentDTO> recordList;
    private PositionAppointmentDTO selectedRecord;
    private PositionAppointmentDTO editRecord;
    ...
    private PositionAppointmentService service;

    public PositionAppointmentMBean() {
        service = new PositionAppointmentService();
    ...
    private void initSearchQuery(boolean isForInit, boolean isForSearch, boolean shouldFetch) {
        ...
        countQueryBuilder.append("select count(d.record_id) from setup_position_appointment d ")
                .append("where ");

        selectQueryBuilder = new StringBuilder();
        selectQueryBuilder.append("select d.record_id,d.description,d.record_status ")
                .append(" from setup_position_appointment d ")
                .append("where ");
    ...
    public void createAction(ActionEvent event) {
        ...
        boolean validation_outcome = validator.validatePositionAppointment(context, newRecord);
        ...
        if (validation_outcome) {
            ...
            try {
                boolean result = service.createPositionAppointment(newRecord);
                if (result) {
                    ViewHelper.addInfoMessage(context, null, "Record Saved Successfully. You may enter another record, or close the dialog.", null);
                    newRecord = null;
                    viewHelper.removeApplicationAttribute(AppConstants.APP_SCOPE_POSITION_APPOINTMENTS);
    ...
    public void prepareUpdateByLink(String row_record_id) {
        if (!StringUtils.isBlank(row_record_id)) {
            try {
                editRecord = service.getPositionAppointment(row_record_id);
    ...
    public void updateAction(ActionEvent event) {
        ...
        RequestContext reqContext = RequestContext.getCurrentInstance();
        boolean validation_outcome = validator.validatePositionAppointment(context, editRecord);
        if (validation_outcome) {
            String activityDate = DateTimeUtils.getActivityDate();
            String currentUser = viewHelper.getAppUser().getUsername();
            ...
            try {
                boolean result = service.updatePositionAppointment(editRecord);
                if (result) {
    ...
    private void fillResults() {
        ...
        try {
            ...
            recordList = (List<PositionAppointmentDTO>) result.get(AppConstants.RECORD_LIST);
    ...
    public List<PositionAppointmentDTO> getRecordList() {
        if (recordList == null) {
    ...
    public void setRecordList(List<PositionAppointmentDTO> recordList) {
    ...
    public PositionAppointmentDTO getNewRecord() {
        if (newRecord == null) {
            newRecord = new PositionAppointmentDTO();
        }
        return newRecord;
    }

    public void setNewRecord(PositionAppointmentDTO newRecord) {
        this.newRecord = newRecord;
    }

    public PositionAppointmentDTO getSelectedRecord() {
        return selectedRecord;
    }

    public void setSelectedRecord(PositionAppointmentDTO selectedRecord) {
        this.selectedRecord = selectedRecord;
    }

    public PositionAppointmentDTO getEditRecord() {
        if (editRecord == null) {
            editRecord = new PositionAppointmentDTO();
        }
        return editRecord;
    }

    public void setEditRecord(PositionAppointmentDTO editRecord) {
