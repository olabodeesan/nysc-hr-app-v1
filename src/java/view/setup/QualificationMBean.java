...
import service.setup.QualificationService;
 ...
 * @author IronHide
 */
public class QualificationMBean implements Serializable{

    private QualificationDTO newRecord;
    private List<QualificationDTO> recordList;
    private QualificationDTO selectedRecord;
    private QualificationDTO editRecord;
    ...
    private void initSearchQuery(boolean isForInit, boolean isForSearch, boolean shouldFetch) {
        ...
        countQueryBuilder.append("select count(d.record_id) from setup_qualifications d ")
                .append("where ");

        selectQueryBuilder = new StringBuilder();
        selectQueryBuilder.append("select d.record_id,d.abbreviation,d.description,d.record_status,d.institution_type ")
    ...
    public void createAction(ActionEvent event) {
        ...
        boolean validation_outcome = validator.validateQualification(context, newRecord);
        ...
        if (validation_outcome) {
            ...
            try {
                boolean result = service.createQualification(newRecord);
                if (result) {
                    ViewHelper.addInfoMessage(context, null, "Record Saved Successfully. You may enter another record, or close the dialog.", null);
                    newRecord = null;
                    viewHelper.removeApplicationAttribute(AppConstants.APP_SCOPE_QUALIFICATIONS);
                ...
                if (StringUtils.equals(msg, AppConstants.DUPLICATE_CODE)) {
                    ViewHelper.addErrorMessage(context, null, "Error: Abbreviation already exists.", null);
                }else if (StringUtils.equals(msg, AppConstants.DUPLICATE_NAME)) {
    ...
    public void prepareUpdateByLink(String row_record_id) {
        if (!StringUtils.isBlank(row_record_id)) {
            try {
                editRecord = service.getQualification(row_record_id);
    ...
    public void updateAction(ActionEvent event) {
        ...
        RequestContext reqContext = RequestContext.getCurrentInstance();
        boolean validation_outcome = validator.validateQualification(context, editRecord);
        if (validation_outcome) {
            String activityDate = DateTimeUtils.getActivityDate();
            String currentUser = viewHelper.getAppUser().getUsername();
            ...
            try {
                boolean result = service.updateQualification(editRecord);
                if (result) {
    ...
    private void fillResults() {
        ...
        try {
            ...
            recordList = (List<QualificationDTO>) result.get(AppConstants.RECORD_LIST);
    ...
    public List<QualificationDTO> getRecordList() {
        if (recordList == null) {
    ...
    public void setRecordList(List<QualificationDTO> recordList) {
    ...
    public QualificationDTO getNewRecord() {
        if (newRecord == null) {
            newRecord = new QualificationDTO();
        }
        return newRecord;
    }

    public void setNewRecord(QualificationDTO newRecord) {
        this.newRecord = newRecord;
    }

    public QualificationDTO getSelectedRecord() {
        return selectedRecord;
    }

    public void setSelectedRecord(QualificationDTO selectedRecord) {
        this.selectedRecord = selectedRecord;
    }

    public QualificationDTO getEditRecord() {
        if (editRecord == null) {
            editRecord = new QualificationDTO();
        }
        return editRecord;
    }

    public void setEditRecord(QualificationDTO editRecord) {
