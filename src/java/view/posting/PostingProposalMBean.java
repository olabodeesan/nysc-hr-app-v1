 ...
 */
public class PostingProposalMBean implements Serializable {
    ...
    public PostingProposalMBean() {
        service = new PostingService();
        viewHelper = new ViewHelper();
        currentDate = DateTimeUtils.getActivityDate();
    ...
    private void initSearchQuery(boolean isForInit, boolean isForSearch, boolean shouldFetch) {
        ...
        if (isForInit) {
            countQueryBuilder.append(" e.employee_rec_id<>'0' ");
            currentCountQuery = countQueryBuilder.toString();

            selectQueryBuilder.append(" e.employee_rec_id<>'0' order by e.current_grade_level desc ");
    ...
    public void searchAction(ActionEvent event) {
        ...
        if (!StringUtils.isBlank(srchGradeLevel)) {
         ...
         if (!StringUtils.isBlank(srchYear)) {
            criteria.add("(year('" + currentDate + "')- year(e.current_date_of_posting_to_location)) >='" 
                    + ViewHelper.fixSqlFieldValue(srchYear) + "'");
        }
