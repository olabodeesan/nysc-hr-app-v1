 ...
 * @author IronHide
 */
public class UpdateEmployeeEmployDataMBean implements Serializable{
    ...
    public UpdateEmployeeEmployDataMBean() {
    ...
    public void updateEmploymentDataAction(ActionEvent event) {
        ...
        if (validation_outcome) {
            ...
            if (retirementDates.isOutcome() == true) {
                ...
                try {
                    boolean result = service.updateEmplyeeEmploymentData(recordDto);
                    if (result) {
                        inEditMode = false;
                        loadRecord();
